interface CalificacionStepperProps {
  activeStep: number;
  steps: string[];
}

const CalificacionStepper = ({ activeStep, steps }: CalificacionStepperProps) => {
  return (
        ...
        return (
          <div key={step} className="flex-1 min-w-0">
              ...
              {!isLast && (
                <div
                  className={`h-0.5 flex-1 ${
                    isCompleted ? 'bg-primary' : 'bg-stroke dark:bg-strokedark'
                  }`}
                />
              )}
            </div>
            ...
            <div className="mt-3 text-center text-xs font-medium uppercase tracking-[0.08em] text-meta-3 dark:text-meta-2">
              <span className={`${isActive ? 'font-semibold text-primary' : isCompleted ? 'text-primary' : ''}`}>
                {step}
              </span>
            </div>
          </div>
...
export default CalificacionStepper;